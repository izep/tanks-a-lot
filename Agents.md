...
## Project snapshot
- **Tech Stack:** TypeScript targeting ES2020, Phaser 3 game engine (Arcade physics) for rendering/loop/input, Vite for bundling, basic PWA shell with service worker, Vitest for unit tests, Live Server for local preview, PowerShell 7 for shell access.
- **Key Paths**
  - `src/phaser/` – Phaser scenes and entry point (mainPhaser.ts)
  - `src/game/` – GameEngine (headless game logic for Phaser integration)
  ...
  - `dist/phaser/` – bundled Phaser code (generated by Vite)
...
- Build: ``npm run build`` (TypeScript compile + Vite bundle + copy static assets)
- Dev server: ``npm run dev`` (Vite dev server with HMR for Phaser development)
